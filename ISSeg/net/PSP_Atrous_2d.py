...
def build_net(tensor_in, BN_FLAG, BATCHSIZE, CLASSNUM, KEEPPROB, IMGCHANNEL):
    print(tensor_in.get_shape())
    input_shape = tensor_in.get_shape().as_list()
    IMGWIDTH = input_shape[1]
    IMGHEIGHT = input_shape[2]
    ...
    with tf.variable_scope('s2u6_br1_pool'):
        s2u6br1_pool = max_pool(s2u5_sum, ksize=2, stride=2)
    ...
    with tf.variable_scope('s2u6_br1_deconv'):
        deconv_res = conv2d_transpose_layer(kernel_size=2, in_put=s2u6br1_conv, out_shape=shape_s2u5,
    ...
    with tf.variable_scope('s2u6_br2_pool'):
        s2u6br2_pool = max_pool(s2u5_sum, ksize=4, stride=4)
    ...
    with tf.variable_scope('s2u6_br2_deconv'):
        deconv_res = conv2d_transpose_layer(kernel_size=4, in_put=s2u6br2_conv, out_shape=shape_s2u5,
    ...
    with tf.variable_scope('s2u6_br3_pool'):
        s2u6br3_pool = max_pool(s2u5_sum, ksize=8, stride=8)
    ...
    with tf.variable_scope('s2u6_br3_deconv'):
        deconv_res = conv2d_transpose_layer(kernel_size=8, in_put=s2u6br3_conv, out_shape=shape_s2u5,
                                                 in_channel=128, out_channel=256, c_stride=8, name='Deconv')
        bn_res = tf.layers.batch_normalization(deconv_res, momentum=0.9, training=BN_FLAG, name='BatchNorm')
        s2u6br3_deconv = tf.nn.relu(bn_res)

    with tf.variable_scope('s2u6_concat'):
        s2u6_concat = tf.concat([s2u5_sum, s2u6br1_deconv, s2u6br2_deconv, s2u6br3_deconv], axis=3, name='concat')
        # channel: 1024
    ...
    with tf.variable_scope('s3_u5_deconv'):
        ...
        deconv = conv2d_transpose_layer(kernel_size=4, in_put=s3u4_sum,
                                             out_shape=[BATCHSIZE, IMGHEIGHT, IMGWIDTH, 64],
    ...
    # shape [BS, height, width, CLASSNUM]

    with tf.variable_scope('SoftMax'):
        pred_annot = tf.argmax(s3u6_conv, axis= 3, name='prediction')

    return s3u6_conv, tf.expand_dims(pred_annot, axis= 3)
...
def conv_layer(kernel_size, in_put, in_channel, out_channel, c_stride = 1, c_rate = 1, name = None):
    with tf.variable_scope(name):
        ...
        conv = tf.nn.conv2d(in_put, conv_weights, strides= [1, c_stride, c_stride, 1], padding= 'SAME', dilations= [1, c_rate, c_rate, 1])
...
def conv2d_transpose_layer( kernel_size, in_put, out_shape, in_channel, out_channel, c_stride, name):
    with tf.variable_scope(name):
        ...
        deconv = tf.nn.conv2d_transpose(in_put, deconv_weights, out_shape, [1, c_stride, c_stride, 1], padding= 'SAME')
